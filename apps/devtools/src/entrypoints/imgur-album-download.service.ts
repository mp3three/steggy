...
export class ImgurAlbumDownloadService implements iRepl {
  constructor(
    ...
    @InjectConfig('DOWNLOAD_TARGET')
    private readonly root: string = '~/Downloads',
    @InjectConfig('NUMBER_PAD_SIZE')
    private readonly padSize: number = PAD_SIZE,
  ) {
  ...
  private processDir(DIR: string) {
    ...
    this.screen.print(`Completed processing ${files.length} files`);
  }
