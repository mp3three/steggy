...
describe('schema', () => {
  ...
  beforeAll(async () => {
    ...
    schemaService = moduleReference.get(FormService);
  });
  ...
  describe('find', () => {
    ...
    it('should be able to findOne by name', async () => {
      ...
      const created = await schemaService.create(form);
      expect(created._id).toBeDefined();
      const found = await schemaService.findByName(created);
      expect(found._id).toStrictEqual(created._id);
    });
    ...
    it('should be able to find many', async () => {
      ...
      const results = await schemaService.findMany({ action });
